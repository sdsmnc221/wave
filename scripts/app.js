$(document).ready(function () {
    ...
    */
    ...
    //Temp badges
    $('.controller__temp').on('click', function (e) {
        $('.controller__temp').removeClass('badge-info').addClass('badge-light').prop('disabled', false);
        $(e.target).removeClass('badge-light').addClass('badge-info').prop('disabled', true);
        if ($(e.target).text().includes('C')) {
            tempConvert(false);
        } else {
            tempConvert();
        }
    ...
    function tempConvert() {
        var toC = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;

        $('.card__infobox--temp').each(function (index, node) {
            var p = $(node).find('p').first(),
